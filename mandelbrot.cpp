...
static Color
mandelbrotPixel(int px, int py, int width, int height,
...
static void
renderMandelbrot(std::vector<Color> &pixels, int width, int height,
    const FractalCamera &camera, int maxIterations)
{
    parallelRender(pixels, width, height, [&](int px, int py) {
        return mandelbrotPixel(px, py, width, height, camera, maxIterations);
    });
}
...
main()
{
    ...
    while (!WindowShouldClose()) {
        ...
        if (needsRender) {
            renderMandelbrot(pixels, WIDTH, HEIGHT, camera, maxIterations);
