...
treeRecursive(Point start, long double length, long double angle, int depth,
    long double leftAngle, long double rightAngle, long double leftScale,
    long double rightScale, std::vector<Segment> &segments)
{
    if (depth == 0) return;
    ...
    treeRecursive(end, length * leftScale, angle - leftAngle, depth - 1,
        leftAngle, rightAngle, leftScale, rightScale, segments);

    treeRecursive(end, length * rightScale, angle + rightAngle, depth - 1,
        leftAngle, rightAngle, leftScale, rightScale, segments);
}
...
fractalTree(int depth)
{
    ...
    treeRecursive({0.0L, 1.5L}, 1.0L, -M_PI / 2.0L, depth, 0.36L, 0.34L, 0.74L,
        0.73L, segments);
