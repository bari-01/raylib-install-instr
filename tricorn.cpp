...
static Color
tricornPixel(int px, int py, int width, int height, const FractalCamera &camera,
    int maxIterations)
{
    double x0 =
...
static void
rendertricorn(std::vector<Color> &pixels, int width, int height,
    const FractalCamera &camera, int maxIterations)
{
    parallelRender(pixels, width, height, [&](int px, int py) {
        return tricornPixel(px, py, width, height, camera, maxIterations);
    });
}
...
main()
{
    ...
    InitWindow(WIDTH, HEIGHT, "rricorn Set");
    SetTargetFPS(165);
    ...
    while (!WindowShouldClose()) {
        ...
        if (needsRender) {
            rendertricorn(pixels, WIDTH, HEIGHT, camera, maxIterations);
        ...
        DrawText("Tricorn Set", 20, 15, 28, WHITE);
