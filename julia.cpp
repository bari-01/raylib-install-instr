...
static Color
juliaPixel(int px, int py, int width, int height, const FractalCamera &camera,
    int maxIterations)
{
    double x =
...
static void
renderJulia(std::vector<Color> &pixels, int width, int height,
    const FractalCamera &camera, int maxIterations)
{
    parallelRender(pixels, width, height, [&](int px, int py) {
        return juliaPixel(px, py, width, height, camera, maxIterations);
    });
}
...
main()
{
    ...
    while (!WindowShouldClose()) {
        ...
        if (needsRender) {
            renderJulia(pixels, WIDTH, HEIGHT, camera, maxIterations);
        ...
        DrawText("Mandelbrot Set", 20, 15, 28, WHITE);
